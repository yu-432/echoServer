...
std::vector<std::string>& HttpHeader::make_response404() {
  static std::vector<std::string> header = {
      ...
      "\r\n",
      "<html><body><h1>404 Not found</h1><p>The requested URL was not found on "
      "this server.</p><hr><address>Original "
...
std::vector<std::string>& HttpHeader::make_response302(std::string path) {
  std::ostringstream oss;
  if (path != "") {
    oss << "Location: https://127.0.0.1:5101" << path.c_str() << "\r\n";
  } else {
    oss << "Location: https://127.0.0.1:5101\r\n";
  }
  static std::vector<std::string> header = {"HTTP/1.1 302 Found\r\n", oss.str(),
                                            "\r\n"};
  return header;
}
